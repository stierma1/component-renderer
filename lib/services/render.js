...
class RenderService{
  ...
  render({renderTmpl, transformedData, params}){
    ...
    if(format === "string" || !format){
      return {format:returnFormat,  renderData:this.renderString(renderString, {params:params, data:transformedData, env:systemConfig.environment})}
    }
    switch(format){
      case "dustjs":
      case "dust":
      case "dust.js": return {format:returnFormat,  renderData:this.renderDust(renderString, {params:params, data:transformedData, env:systemConfig.environment})}
    }
  }
